...
export const POPULAR_CONTENT_TITLE = 'Popular Content'
export const LATEST_TRANSACTIONS = 'Latest Transactions'

const isPopularContent = (title: string) => {
  return title === POPULAR_CONTENT_TITLE
}

const CardList = ({ title }: { title: string }) => {
  const list = isPopularContent(title) ? popularContent : latestTransactions

  return (
    <div>
      <div className="text-lg font-medium mb-6">{title}</div>
      <div className="flex flex-col gap-2">
        {list.map(({ id, title, image, badge, count }) => (
          <Card
            key={id}
          ...
          >
            ...
            <CardFooter className="p-0">{count / 1000}K</CardFooter>
          </Card>
        ))}
      </div>
    </div>
